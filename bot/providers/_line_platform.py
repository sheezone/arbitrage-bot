...
def parse_line_dump(raw: dict, game_to_categories: dict[str, list[int]], bookmaker: str) -> list[SourceQuote]:
    category_to_game = {cat: game for game, cats in game_to_categories.items() for cat in cats}

    segment_to_game = {
        s["id"]: category_to_game[s["sportCategoryId"]]
        for s in raw.get("sports", [])
        if s.get("kind") == "segment" and s.get("sportCategoryId") in category_to_game
    }
