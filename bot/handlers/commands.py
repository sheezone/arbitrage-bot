...
def _help_view() -> View:
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Сервис сравнивает коэффициенты российских лицензированных букмекеров "
        "(реестр ЕРАИ / ЦУПИС) и показывает расхождения в котировках на "
        "двухисходные события — это <b>информационно-аналитический инструмент</b>, "
        "а не гарантия выигрыша.\n\n"
        "<b>Пример расчёта:</b> БК1 — 2.10 на победу команды А, БК2 — 2.05 на "
        "победу команды Б. Бот рассчитывает, как распределился бы банкролл между "
        "исходами. Итог зависит от того, останутся ли коэффициенты прежними к "
        "моменту ставки, примет ли БК ставку и не изменит ли условия — реальный "
        "результат сервис не гарантирует.\n\n"
        ...
        "самостоятельно у лицензированных букмекеров и несёте все связанные риски. "
        "Использование сервиса означает согласие с офертой и политикой обработки "
        "персональных данных."
    )
...
def register_handlers(
    ...
    async def on_sub_pay_sbp(callback: CallbackQuery, plan: billing.Plan, bot: Bot) -> None:
        if not sbp_enabled:
            await callback.answer("Оплата через СБП пока не подключена", show_alert=True)
            return
        chat_id = callback.message.chat.id
        ...
        # order_id round-trips through Prodamus untouched -- pack everything we need to
        # credit the right user back out of the webhook (see /api/prodamus/webhook).
        order_id = f"sbp-{chat_id}-{plan.id}-{int(time.time())}"
        product_name = f"Подписка на {plan.label} — Арбитражный бот"
        if discount_used > 0:
            product_name += f" (скидка: -{discount_used:.0f} ₽)"
        pay_url = prodamus.build_payment_url(
            prodamus_form_url,
            order_id=order_id,
            amount=discounted,
            product_name=product_name,
            notification_url=f"{webapp_url.rstrip('/')}/api/prodamus/webhook",
            success_url=f"https://t.me/{bot_username}" if bot_username else "",
            customer_extra=f"chat_id={chat_id}",
    ...
    async def on_sbp_check(callback: CallbackQuery, bot: Bot) -> None:
        # The webhook is the source of truth and does the crediting -- this button only
        # reports whether it has landed yet (has_payment on the prodamus:<order_id> key).
        _, order_id, _plan_id = callback.data.split(":", 2)
        if not repo.has_payment(f"prodamus:{order_id}"):
    ...
    # button. Both go through bot/core/payments.py (idempotent on the payment id).
    ...
    async def on_yksbp_check(callback: CallbackQuery, bot: Bot) -> None:
        ...
        if not repo.has_payment(f"yookassa_sbp:{payment_id}"):
            ...
            if not yookassa_is_paid(payment):
                await callback.answer("Оплата ещё не поступила. Попробуйте через минуту.", show_alert=True)
                return
            if credit_yookassa_sbp(repo, chat_id, payment) is None:
